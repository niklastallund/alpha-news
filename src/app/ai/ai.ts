...
export async function dbg_focusOnImage() : Promise<ResultPatternType<GeneratedArticle>>  {
 ...
 */
async function generateImageForArticle(article: GeneratedArticleBase): Promise<ResultPatternType<string>> {
    ...
    try {
        ...
        if (!apiKey) {
             return {success: false, msg: "AZURE_API_KEY or AZURE_RESOURCE_ENDPOINT is not set in environment variables."}
        }
...
async function genPureArticle(prompt:string, category:string, temp:number = 0.7) : Promise<ResultPatternType<string>> {
 ...
 * @returns 
 */
async function gererateNewsObject(prompt:string, category: string, temp:number = 0.7) : Promise<ResultPatternType<GeneratedArticle>> {
    ...
    try {
        ...
        return {success: false, msg: "Object article failed. " + String(errorMsg)}
    }
 ...
 */
export async function generateArticle(prompt:string, category: string, temp:number, genImg:boolean = false) : Promise<ResultPatternType<GeneratedArticle>> {


    try {

        const articleBase = await gererateNewsObject(prompt, category, temp);
        if (!articleBase.success) return {success: false, msg: "Could not generate article as obj."}
        ...
        if (genImg) {
            ...
            return {success: true, data: {...articleBase.data, imageUrl: img.data}}
        }
        ...
        return {success: false, msg: "GenerateArticle failed. " + String(errorMsg)}
